...
public class Winlog {
	...
	public Winlog(String log) throws ParseException {
		...
		if (matcher.find()) {
			...
			String logleft = log.substring(0, log.indexOf(Securitymatcher.group(0))+Securitymatcher.group(0).length());
			String [] loglefts = logleft.split(" ");
			this.hostname = loglefts[3];
			if (ipmatcher.find()) {
	...
	public <T> String getClassMapping(T classes) {
		...
		String [] fielddata = {"userid","deptid","equipmentid","logtime","logname","ip","logsource","eventid","tasktype","operation_level","keywords","hostname","logtime_year","logtime_month","logtime_day","logtime_hour","logtime_minute","equipmentname","event_type"};
      ...
      for (int i = 0; i < fields.length; i++) {
           ...
           if (fields[i].getName().equals("operation_des")||fields[i].getName().equals("keywords")||fields[i].getName().equals("hostname")||fields[i].getName().equals("ip")||fields[i].getName().equals("equipmentname")||fields[i].getName().equals("event_des")) {
	...
	public static void main(String[] args) throws IOException {
		...
		//String log = "<29> 2018-03-27 14:03:49 MCTC-ZSC 192.168.20.72 Security: 520: NT AUTHORITY\\LOCAL SERVICE: 系统时间已更改。 进程 ID: 844 进程名: C:\\WINDOWS\\system32\\svchost.exe 主要用户名: LOCAL SERVICE 主要域: NT AUTHORITY 主要登录 ID: (0x0,0x3E5) 客户端用户名: - 客户端域: - 客户端登录: - 上一时间: 14:04:28 2018-3-28 新时间: 14:03:49 2018-3-28 ";
