...
public class AopAspectJ {
	 ...
	 */
//	public static final String EDP = "execution(*  com.jz.bigdata..*.controller.*.*(..))";
	
	@Pointcut("execution(*  com.jz.bigdata..*.controller.*.*(..))")
    public void myMethod(){};

    
//    @Pointcut("execution(*  com.jz.bigdata.common.*.controller.*.*(..))||execution(*  com.jz.bigdata.business.logAnalysis.log.controller.*.*(..))")
//    public void myMethod(){};
//    
	...
	public void doBefore(JoinPoint jp) throws ClassNotFoundException {
			...
			//退出登录
//			if (jp.getSignature().getName().equals("logout") && (jp.getArgs()[0]).toString().equals("999")) {
			if (jp.getSignature().getName().equals("loginOut")&&session.getAttribute(Constant.SESSION_USERID)!=null) {
				...
				if(session.getAttribute(Constant.SESSION_DEPARTMENTID)==null){
					describeSrt = "用户账号:" + session.getAttribute(Constant.SESSION_USERACCOUNT).toString()  + "    " + "操作:" + describe
							+ "    " + "操作状态:成功";
				}else{
...
//			note.setError("登录成功");
		...
		}else if(session.getAttribute(Constant.SESSION_USERID)!=null&&!pjp.getSignature().getName().equals("loginOut")){//所有的不报异常方法
			note.setResult(1);
			note.setUserId(session.getAttribute(Constant.SESSION_USERID).toString());
			String describeSrt ="";
			if(session.getAttribute(Constant.SESSION_DEPARTMENTID)==null){
				 describeSrt =  "用户账号:"+ session.getAttribute(Constant.SESSION_USERACCOUNT).toString() 
						+ "    " + "操作:" + describe
						+ "    " + "操作状态:成功";
			}else{
				note.setDepartmentId(Integer.valueOf(session.getAttribute(Constant.SESSION_DEPARTMENTID).toString()));
	...
	public void doAfterThrowing(JoinPoint jp, Exception ex) throws ClassNotFoundException {
		...
		if(describe.equals("用户登录")){//用户登录报异常
			...
			if(session.getAttribute(Constant.SESSION_DEPARTMENTID)==null){
				describeSrt =  "用户账号:"
						+ session.getAttribute(Constant.SESSION_USERACCOUNT).toString() +  "    " + "操作:" + describe + "    "
						+ "操作状态:失败" ;
				
			}else{
				note.setDepartmentId(Integer.valueOf(session.getAttribute(Constant.SESSION_DEPARTMENTID).toString()));
				describeSrt =  "用户账号:"
						+ session.getAttribute(Constant.SESSION_USERACCOUNT).toString() + "    " + "部门:"
						+ session.getAttribute(Constant.SESSION_DEPARTMENTNAME).toString() + "    " + "操作:" + describe + "    "
						+ "操作状态:失败" ;
			}
			note.setDescribe(describeSrt);
	 ...
	 * @param jp
	 */
	private static void initializeData(JoinPoint jp) {
